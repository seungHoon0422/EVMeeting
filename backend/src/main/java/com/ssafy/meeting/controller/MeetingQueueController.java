...
public class MeetingQueueController {
    @Autowired
    MeetingQueueService callService;
...
//    @ApiResponse()
    public String join(@RequestBody @ApiParam(value="접속 정보", required = true) MeetingQueue meetingQueue){
        ...
        if(!list.isEmpty()){
            ...
            MeetingQueue temp = callService.createMeeting(meetingQueue);
            return meetingQueue.getSessionid(); // 생성한 세션아이디를 리턴 -> 이때는 세션아이디를 테이블에서 지우면 안된다
        }
